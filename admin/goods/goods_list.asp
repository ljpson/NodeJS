...
DbRec.CursorType=1
if keyword="" then  
  DbRec.Open "SELECT * FROM goods_info where nshop_code='"&session("nshop_id")&"' and del='Y' ORDER BY "&sort&" "&order , DbCon
else
  DbRec.Open "SELECT * FROM goods_info where nshop_code='"&session("nshop_id")&"' and del='Y' and "&search&" like '%"&keyword&"%' ORDER BY "&sort&" "&order , DbCon
end if
...
<html>
<head><title>[NPOINT] 관리자툴 - NSHOP 상품목록</title></head>
...
<%
	...
	Set rsnn=DbCon.Execute(Sqlnn) %>			
				<tr>
					<td align=center colspan=2><font size=3 color="000000"><b>[<%=rsnn("nshop_kname")%>] 상품목록</b></font><br><Br></td>
				</tr>
				<tr>
					<td>
						<table width=100%>
...
<%	sql_ca="SELECT * FROM goods_category where goods_code='"&DbRec("goods_code")&"' ORDER BY goods_category"
	...
	if rs_ca.eof then
		ca="카테고리 미등록" %>
										<tr>
											<td><%=ca%></td>
											<td width=90 align=right><INPUT type="button" value="카테고리추가" style="BACKGROUND-COLOR:#cccccc; COLOR:#222222; width:84px; height:18px; BORDER:1x solid #778899; cursor:hand;" onclick="window.open('goods_category_input.asp?goods_code=<%=DbRec("goods_code")%>', 'category_input', 'width=400,height=160') ;"></td>
										</tr>
<%	else
		do until rs_ca.eof
			...
			set rs_ca3 = nothing %>
										<tr>
											<td width=35><INPUT type="button" value="삭제" style="BACKGROUND-COLOR:#cccccc; COLOR:#222222; width:30px; height:18px; BORDER:1x solid #778899; cursor:hand;" onclick="javascript:if (confirm('카테고리를 정말로 삭제하시겠습니까?')) {location.replace('goods_category_del_result.asp?goods_code=<%=rs_ca("goods_code")%>&goods_category=<%=rs_ca("goods_category")%>&page=<%=curpage%>&startpage=<%=startpage%>&re=list');}"></td>
											<td>
	...
	<%	rs_ca.movenext
		loop %>
										<tr>
											<td colspan=2 align=right><INPUT type="button" value="카테고리추가" style="BACKGROUND-COLOR:#cccccc; COLOR:#222222; width:84px; height:18px; BORDER:1x solid #778899; cursor:hand;" onclick="window.open('goods_category_input.asp?goods_code=<%=DbRec("goods_code")%>', 'category_input', 'width=400,height=160') ;"></td>
										</tr>
<%	end if
	set rs_ca = nothing	 %>
									</table>
										...
										<tr>
											<td align=left> 
												<table>
													...
													<form action="goods_list.asp?sort=<%=sort%>&order=<%=order%>" method=post>
											...
											<td align=right>
												<table>
													<tr>
													<form action=goods_list.asp method=post>
														<td> 
						...
						<table border="0" width=100%>
							<tr>
								...
								<td width=40% align=center>
        							<table border="0">
										<form name="form4" method="Post" action="goods_list.asp">                  			
				    					<tr>
				    						<td class="font9">
...
<%if totpage > ten then%>
	<%if startpage=1 then%>
						<font size="2">◁◁</font>&nbsp;
    <%else%>
						<a class="spage" href="goods_list.asp?search=<%=search%>&keyword=<%=keyword%>&sort=<%=sort%>&order=<%=order%>&page=<%=cint(startpage)-ten%>&startpage=<%=cint(startpage)-ten%>"><font size="2">◀◀</font></a>&nbsp;
    <%end if
    ...
    For a=startpage to startpage+ten-1
		if a>totpage then
			...
			<%	else	%>
						<font color="#000000" size="2"><a href="goods_list.asp?search=<%=search%>&keyword=<%=keyword%>&sort=<%=sort%>&order=<%=order%>&page=<%=a%>&startpage=<%=startpage%>"><%=a%></a></font>
			<%	End if
		end if
    Next
    %>
    ...
    <%if((startpage\ten)=(totpage\ten)) then%> 
						&nbsp;<font size="2">▷▷</font>
    <%else%>
						&nbsp;<a class="spage" href="goods_list.asp?search=<%=search%>&keyword=<%=keyword%>&sort=<%=sort%>&order=<%=order%>&page=<%=a%>&startpage=<%=a%>"><font size="2">▶▶</font></a>
    <%end if%>
...
<%else%>
	...
	%>
						<font color="#ff0000" size="2"><b><%=a%></b></font>
	<%	else%>
						<a href="goods_list.asp?search=<%=search%>&keyword=<%=keyword%>&sort=<%=sort%>&order=<%=order%>&page=<%=a%>"><font color="#000000" size="2"><%=a%></font></a>
	<%	end if
    next
    %>
						&nbsp;<font color="#808000" size="2">▷▷</font>
